...
async def lifespan(app:FastAPI):
    ...
    print("🔥 Ember Gateway v9 (Convex向量记忆版) 启动中...")
    try:
        if REDIS_HOST:
    ...
    print(f"🔢 Embedding: {'✅ 阿里百炼' if DASHSCOPE_API_KEY else '❌ 未配置'}")
...
async def admin_panel(request: Request):
    ...
    if CONVEX_URL:
        try:
            ...
            if stats:
                convex_html = f"""
                <div style="border:1px solid #7c3aed;padding:12px;margin:8px 0;border-radius:8px;background:#1a1a2e">
                    <b>🔮 Convex 向量记忆</b><br>
                    记忆总数: {stats.get('totalMemories', 0)} | 摘要: {stats.get('totalSummaries', 0)} | 置顶: {stats.get('pinnedCount', 0)}<br>
                    维度: {json.dumps(stats.get('byDimension', {}), ensure_ascii=False)}<br>
                    <small style="color:#888">URL: {CONVEX_URL}</small>
                </div>
                """
            else:
                convex_html = '<div style="border:1px solid #e94560;padding:12px;margin:8px 0;border-radius:8px">⚠️ Convex 连接异常</div>'
        except:
            convex_html = '<div style="border:1px solid #e94560;padding:12px;margin:8px 0;border-radius:8px">⚠️ Convex 查询失败</div>'
    else:
        convex_html = '<div style="border:1px solid #555;padding:12px;margin:8px 0;border-radius:8px;color:#888">Convex 未配置（.env 中添加 CONVEX_URL 和 DASHSCOPE_API_KEY）</div>'
...
async def chat_completions(request:Request, background_tasks:BackgroundTasks):
    ...
    if user_content and CONVEX_URL and DASHSCOPE_API_KEY:
        try:
            semantic_memories = await semantic_recall(user_content, limit=5)
            if semantic_memories:

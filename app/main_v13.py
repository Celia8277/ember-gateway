            ...
            async for line in r.aiter_lines():
                ...
                if line.startswith("data: "):
                    ...
                    if data == "[DONE]":
                        ...
                        # flush 缓冲区剩余内容
                        if _filter_buf:
                            filtered = filter_assistant_speak(_filter_buf)
                            if filtered:
                                flush_ev = {"choices": [{"delta": {"content": filtered}, "finish_reason": None}]}
                                yield f"data: {json.dumps(flush_ev, ensure_ascii=False)}\n\n"
                            _filter_buf = ""
                        if full_content:
                    ...
                    try:
                        ...
                        if choices:
                            ...
                            if delta.get("content"):
                                ...
                                # 攒够 20 字符再过滤输出，避免截断正则词
                                if len(_filter_buf) >= 20 or finish_reason:
                                    filtered = filter_assistant_speak(_filter_buf)
                                    ev["choices"][0]["delta"]["content"] = filtered
                                    data = json.dumps(ev, ensure_ascii=False)
                                    _filter_buf = ""
                                else:
                                    # 还没攒够，先不输出这个 chunk 的内容
...
async def data_cleanup_loop():
    ...
    while True:
        try:
            ...
            now = datetime.utcnow().replace(tzinfo=tz.utc).astimezone(timezone(timedelta(hours=8)))
